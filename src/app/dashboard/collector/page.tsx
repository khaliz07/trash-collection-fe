...
import { ArrowRight, BarChart, Clock, MapPin, Truck } from "lucide-react";
...
export default function CollectorDashboard() {
  ...
  const todaysCollections = {
    total: points.length,
    completed: points.filter((cp) => cp.status === CollectionStatus.COMPLETED)
      .length,
    pending: points.filter((cp) => cp.status === CollectionStatus.PENDING)
      .length,
  };
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">
            {t("collector_dashboard.welcome_back")}
          </h2>
      ...
      <Tabs defaultValue="today" className="space-y-4">
        <TabsList>
          <TabsTrigger value="today">
            {t("collector_dashboard.tabs.today")}
          </TabsTrigger>
          <TabsTrigger value="performance">
            {t("collector_dashboard.tabs.performance")}
          </TabsTrigger>
          <TabsTrigger value="urgent">
            {t("collector_dashboard.tabs.urgent")}
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {todaysCollections.total}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t(
                    "collector_dashboard.stats.todays_collections.description"
                  )}
                </p>
                <div className="mt-4">
                ...
                <div className="mt-1 text-xs flex justify-between">
                  <div>
                    {todaysCollections.completed}{" "}
                    {t(
                      "collector_dashboard.stats.todays_collections.completed"
                    )}
                  </div>
                  <div>
                    {todaysCollections.pending}{" "}
                    {t(
                      "collector_dashboard.stats.todays_collections.remaining"
                    )}
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-2 text-xs">
                  {t("collector_dashboard.stats.route_progress.est_completion")}
                  : 2:30 PM
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t("collector_dashboard.stats.efficiency_rate.description")}:
                  11 min
                </p>
                <div className="mt-4">
                  <div className="h-1 w-full bg-muted rounded-full overflow-hidden">
                ...
                <div className="mt-2 text-xs text-emerald-600 dark:text-emerald-400">
                  +5%{" "}
                  {t(
                    "collector_dashboard.stats.efficiency_rate.from_last_week"
                  )}
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                ...
                >
                  <Button variant="outline" size="sm">
                    {t(
                      "collector_dashboard.stats.urgent_requests.view_requests"
                    )}
                  </Button>
                </Link>
              </CardContent>
        ...
        <TabsContent value="performance">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("collector_dashboard.performance_analytics.title")}
              </CardTitle>
              <CardDescription>
                {t("collector_dashboard.performance_analytics.description")}
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="urgent">
          <Card>
            <CardHeader>
              <CardTitle>
                {t("collector_dashboard.urgent_requests.title")}
              </CardTitle>
              <CardDescription>
                {t("collector_dashboard.urgent_requests.description")}
              </CardDescription>
            </CardHeader>
            <CardContent>
