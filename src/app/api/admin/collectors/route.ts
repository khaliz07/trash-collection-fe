...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Check CCCD uniqueness (if CCCD is being changed)
    if (cccd && cccd !== existingCollector.cccd) {
      const cccdExists = await prisma.user.findFirst({
        where: {
          cccd,
          role: "COLLECTOR",
  ...
  try {
    ...
    if (existingCollector.status === "ACTIVE") {
      return NextResponse.json(
        {
          message:
            "Không thể xóa nhân viên đang hoạt động. Vui lòng tạm ngưng trước khi xóa.",
        },
        { status: 400 }
      );
    }
