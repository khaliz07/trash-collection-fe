...
function getUserId(request: NextRequest): string {
  ...
  return 'user-1'; // Default for demo
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const packageData = {
      ...
      startDate: subscription.activatedAt?.toISOString() || subscription.createdAt.toISOString(),
      endDate: endMonth ? `${endMonth}-28` : new Date().toISOString(),
      status,
      fee: Number(subscription.package?.price || 80000),
      area: subscription.user.address || 'Phường Linh Trung, Quận Thủ Đức, TP. HCM',
      description: subscription.package?.description || 'Thu gom rác 2 lần/tuần (Thứ 3 và Thứ 6)',
      ...
      collectionsTotal: (subscription.package?.collectionsPerWeek || 2) * (subscription.package?.duration || 1) * 4,
